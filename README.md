...
## ¿Sos principiante? Empezá acá

Tres ideas para no perderte:

1. **La VM se escribe en Smalltalk, no en C.** El código C de `src/` está
   **generado** por una herramienta (VMMaker/Slang) desde una imagen Squeak.
   Dice `DO NOT EDIT THIS FILE` arriba de cada archivo: hacele caso. Este fork
   solo *compila* ese C; no incluye lo necesario para *regenerarlo*.
2. **El código que sí se escribe a mano** es el "pegamento" de cada sistema
   operativo, en `platforms/`. Si querés tocar algo, casi siempre es ahí.
3. **El build no usa CMake** (aunque parezca). Cada plataforma se compila con su
   script `mvm` en `building/<target>/squeak.cog.spur/`.

Mapa mínimo de dónde vive cada cosa:

| Carpeta | Qué es | ¿La toco? |
|---------|--------|-----------|
| `src/` | C generado (la VM: intérprete + JIT) | **No** (es generado) |
| `platforms/Cross/` | pegamento común a todas las plataformas | Sí, con cuidado |
| `platforms/{unix,win32,iOS}/` | pegamento de Linux / Windows / macOS | Sí, con cuidado |
| `building/<target>/` | scripts de build por plataforma (`mvm`) | Para ajustar el build |
| `scripts/ci/` | lo que corre GitHub Actions | Para tocar el CI |
| `third-party/` | dependencias externas | No (son de terceros) |

Para compilar, seguí [Compilar localmente](#compilar-localmente). Si algo falla,
el detalle por plataforma está en `building/<target>/HowToBuild`.
